...
from PyQt6.QtGui import QPainter, QColor, QPen, QPainter
...
class DungeonWidget(QWidget):
            ...
            for x in range(self.GRID_SIZE):
                if self.grid[y][x] == 1:
                    painter.fillRect(
                        ...
                        self.CELL_SIZE,
                        self.CELL_SIZE,
                        QColor(100, 100, 100)  # Gray color for rooms and corridors
                    )
                # Draw cell borders
    ...
    def generate_corridor(self, room1: Tuple[int, int, int, int], room2: Tuple[int, int, int, int]):
